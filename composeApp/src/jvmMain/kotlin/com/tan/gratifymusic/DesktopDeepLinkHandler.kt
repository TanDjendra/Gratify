 ...
 * - gratifymusic://open-app?url=<encoded_url>  (redirected from website)
 * - gratifymusic://watch?v=VIDEO_ID            (direct scheme)
 * - gratifymusic://playlist?list=PLAYLIST_ID   (direct scheme)
 * - gratifymusic://channel/CHANNEL_ID          (direct scheme)
 * - gratifymusic://album?id=ALBUM_ID           (direct scheme)
 * - https://gratifymusic.org/app/...            (web URL passed via args)
 */
object DesktopDeepLinkHandler {
    private const val TAG = "DesktopDeepLinkHandler"

    private val pendingUriFile: File by lazy {
        File(System.getProperty("java.io.tmpdir"), "gratifymusic_pending_deeplink.txt")
    }
     ...
     *
     * Conversion rules:
     * 1. gratifymusic://open-app?url=<encoded_url>
     *    → Extract the `url` param and use it as intent data
     *
     * 2. gratifymusic://watch?v=xxx, gratifymusic://playlist?list=xxx, etc.
     *    → Convert to https://gratifymusic.org/app/watch?v=xxx format
     *      so App.kt handles it uniformly via the gratifymusic.org branch
     *
     * 3. https://gratifymusic.org/app/... or YouTube URLs
     *    → Pass through as-is
     */
    private fun parseToIntent(uri: String): GenericIntent {
        ...
        val actualUri = when {
            // gratifymusic://open-app?url=<encoded_url>
            parsed.scheme == "gratifymusic" && parsed.host == "open-app" -> {
                val urlParam = parsed.getQueryParameter("url")
                if (urlParam != null) {
                    Logger.d(TAG, "Extracted URL from open-app: $urlParam")
                    Uri.parse(urlParam)
                } else {
                    // gratifymusic://open-app without params → just open the app, no navigation
                    Logger.d(TAG, "open-app without URL param, just opening app")
                    null
                }
            }

            // gratifymusic://watch?v=xxx → https://gratifymusic.org/app/watch?v=xxx
            // gratifymusic://playlist?list=xxx → https://gratifymusic.org/app/playlist?list=xxx
            // gratifymusic://channel/UCxxx → https://gratifymusic.org/app/channel/UCxxx
            // gratifymusic://album?id=xxx → https://gratifymusic.org/app/album?id=xxx
            parsed.scheme == "gratifymusic" && parsed.host != null -> {
                val host = parsed.host!!
                val query = parsed.query?.let { "?$it" } ?: ""
                val pathSuffix = parsed.pathSegments.joinToString("/").let {
                    if (it.isNotEmpty()) "/$it" else ""
                }
                val convertedUrl = "https://gratifymusic.org/app/$host$pathSuffix$query"
                Logger.d(TAG, "Converted gratifymusic:// to: $convertedUrl")
                Uri.parse(convertedUrl)
            }

            // https://gratifymusic.org/app/... or YouTube URLs → pass through
            else -> parsed
        }
