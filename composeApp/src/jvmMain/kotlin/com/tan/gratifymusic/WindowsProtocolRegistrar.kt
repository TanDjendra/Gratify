...
/**
 * Registers the "gratifymusic://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\gratifymusic
 *     (Default) = "URL:GratifyMusic Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "gratifymusic"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering gratifymusic:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:GratifyMusic Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\GratifyMusic.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/GratifyMusic.exe
        // So we go: java.home → parent (runtime) → parent (app) → GratifyMusic.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "GratifyMusic.exe")
            if (exeFile.exists()) {
