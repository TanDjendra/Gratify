...
object CrashDialog {
    ...
    private fun showCrashDialog(thread: Thread, throwable: Throwable) {
        ...
        val versionInfo = try {
            "GratifyMusic Desktop v${VersionManager.getVersionName()}"
        } catch (_: Exception) {
            "GratifyMusic Desktop"
        }

        val dialog = JDialog().apply {
            title = "GratifyMusic - Unexpected Error"
            isModal = true
        ...
        val headerPanel = JPanel(BorderLayout(8, 4)).apply {
            val titleLabel = JLabel("GratifyMusic has crashed").apply {
