...
import com.tan.domain.data.entities.PodcastWithEpisodes
...
import com.tan.domain.data.entities.analytics.query.TopPlayedAlbum
import com.tan.domain.data.entities.analytics.query.TopPlayedArtist
import com.tan.domain.data.entities.analytics.query.TopPlayedTracks
...
interface DatabaseDao {
    ...
    @Insert
    suspend fun insertNotificationRaw(notificationEntity: NotificationEntity)

    @Query("DELETE FROM notification WHERE id NOT IN (SELECT id FROM (SELECT id FROM notification ORDER BY time DESC LIMIT 100))")
    suspend fun pruneOldNotifications()

    @Transaction
    suspend fun insertNotification(notificationEntity: NotificationEntity) {
        insertNotificationRaw(notificationEntity)
