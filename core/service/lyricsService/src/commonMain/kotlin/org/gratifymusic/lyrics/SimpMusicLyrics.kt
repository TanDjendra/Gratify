...
import org.gratifymusic.lyrics.models.request.VoteBody

class GratifyMusicLyrics {
    ...
    private val baseUrl = "https://api-lyrics.gratifymusic.org/v1/"
    ...
    private fun createClient() =
        HttpClient(getEngine()) {
            ...
            install(HttpCache)
            install(CurlLogger) {
                logger = { Logger.d("GratifyMusicLyrics", it) }
            }
            install(HttpSend) {
                maxSendCount = 100
            }
            ...
            defaultRequest {
                url("https://api-lyrics.gratifymusic.org/v1")
            }
            if (proxy != null) {
                engine {
                    proxy = this@GratifyMusicLyrics.proxy
                }
            }
        }
    ...
    private fun HttpRequestBuilder.buildDefaultHeaders(
        ...
        headers {
            ...
            header(HttpHeaders.UserAgent, "GratifyMusicLyrics/1.0")
